...
function selfHosted(p, monthlyTokens){
  ...
  const n = p.gpuCount, tp = p.throughput, util = p.utilization/100;
...
function renderTornado(r) {
  const inputs = [
    {id:"steps", label:"Steps/task"},
    {id:"outPerStep", label:"Output tokens/step"},
    {id:"toolTokens", label:"Tool tokens/step"},
    {id:"sysTokens", label:"System tokens"},
    {id:"userTokens", label:"User tokens"},
    {id:"routeShare", label:"Route share"},
    {id:"cacheHit", label:"Cache hit rate"},
    {id:"tasksPerDay", label:"Tasks/day"}
  ];

  const baseline = r.totalMo;
  const results = inputs.map(inp => {
    const pLow = {...r.p}, pHigh = {...r.p};
    if (inp.id === "cacheHit") {
      pLow[inp.id] = Math.max(0, r.p[inp.id] * 0.8);
      pHigh[inp.id] = Math.min(95, r.p[inp.id] * 1.2);
    } else {
      pLow[inp.id] = r.p[inp.id] * 0.8;
      pHigh[inp.id] = r.p[inp.id] * 1.2;
    }
    const lowCost = compute(pLow).totalMo;
    const highCost = compute(pHigh).totalMo;
    return {
      label: inp.label,
      low: lowCost - baseline,
      high: highCost - baseline,
      impact: Math.abs(highCost - lowCost)
    };
  });
  ...
  const lowData = results.map(x => x.low);
  const highData = results.map(x => x.high);

  if (tornadoChart) {
    tornadoChart.data.labels = labels;
    tornadoChart.data.datasets[0].data = lowData;
    tornadoChart.data.datasets[1].data = highData;
    tornadoChart.update();
  } else {
    tornadoChart = new Chart($("tornadoChart"), {
      ...
      data: {
        labels,
        datasets: [
          {label: "-20%", data: lowData, backgroundColor: "#f87171"},
          {label: "+20%", data: highData, backgroundColor: "#34d399"}
        ]
      },
      options: {
        indexAxis: "y",
        plugins: {
          ...
          tooltip: {callbacks: {label: ctx => (ctx.raw > 0 ? "+" : ctx.raw < 0 ? "-" : "") + fmt(Math.abs(ctx.raw), 0) + "/mo"}}
        },
        scales: {
          x: {ticks: {color: "#8b94a8", callback: v => (v > 0 ? "+" : v < 0 ? "-" : "") + "$" + Math.abs(v)}, grid: {color: "#1f2735"}},
          y: {ticks: {color: "#8b94a8"}, grid: {display: false}}
        },
        maintainAspectRatio: false
      }
    });
